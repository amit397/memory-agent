...
## Experimental Rules (do not break without flagging me)

*Pre-registered 2026-06-25. Binding on all future sessions; each rule keeps its reasoning so it is not quietly undone. Several amend earlier sections (noted inline). To change one, flag it and say why — never silently relax it.*

**R1 — Real test engine before any candidate method.** Do not build, wire, or run any *candidate* compression policy (B1 full retrieval, B2 recency+summary, the Cx semantic methods) until the *real* loop — real model, real tokenizer, one cheap real-model backend — first reproduces the Phase-0 gate. Replace the chars/4 estimate (`harness/loop.py:count_tokens`) with the real tokenizer before *any* token number is recorded or reported. The `RealBackend`/`RealLLMDriver` stubs must be working first. *(B3 / log-everything is the instrument's floor, not a candidate — it runs as part of the gate, see R2.)* **Why:** the methods are measured against this engine; if the engine is fake or the token counts estimated, every comparison measures the engine's quirks, not the methods. Make the instrument real before measuring anything.

**R2 — Go/no-go is overflow AND a real memory dependency, not overflow alone.** Amends the Phase-0 gate (§3, §10). To clear scarcity, run B3 *and* a planted-dependency test. History overflowing is necessary but not sufficient: the agent must *measurably and attributably* degrade — re-propose a known-bad experiment, lose acceptance, or deliver a worse score — when a needed old item is dropped. ("Fail" means measurable attributable degradation, not a crash.) **Why:** overflow full of things the agent never needed is not a real test — you could throw it all away for free and "keep recent" would win, making the whole project look pointless. The overflow has to contain things that matter, or we are answering the wrong question.

**R3 — The planted-dependency test must be able to fail.** Plant a failure early, make it tempting again later (*past* the last-N window), and check whether B3 re-proposes it while a failure-preserving policy does not. Build the temptation to re-propose so it is genuine; never rig it so it can only confirm the hypothesis. (Showing the dependency *bites* needs only B3; the failure-preserving contrast policy is built later, under R1.) **Why:** this is the test that proves memory matters, so it is only worth anything if it could have come out the other way. A test built to pass tells us nothing; a test that genuinely risks failing is the one whose pass we can trust.

**R4 — Attention backend is fixed: SDPA or flash_attention_2, never FA3.** Same backend in every run. No Hopper-only code path. **Why:** the attention backend is part of the model's math. If some runs use one and some another, a difference in results could be the memory method or the backend, and we could not tell which. Hold it constant so the only thing varying is the thing we study. (FA3 is also Hopper-only and won't run on the 8 GB dev box.)

**R5 — All token counts are all-in.** Include the tokens the compressor itself spends making summaries (input *and* output of every summarizer call). Reinforces §7. **Why:** the whole claim is "saves tokens." A method that shrinks the history but burns a pile of tokens writing summaries hasn't saved anything. Counting only the shrunk part would let a method look like a win while secretly costing more.

**R6 — Kill rule, amending Phase 4 (§10).** A Part-1 null kills the project *only if* B3 also passes exact-token retention. If B3 wins the score but fails to keep exact strings (file paths, identifiers, error text), that is NOT a kill — it advances to a properly-powered Part 2, because the autoresearch score is then blind to what coding depends on. Conversely, a B3 win *with* exact-token retention IS a kill — the "only if" is what keeps Part 1's teeth. Operationalize it: exact-token retention is measured by a **pre-committed planted exact-token needle** (§10 Phase 1) whose pass threshold is fixed in advance; "B3 fails exact-token retention" must be a measured fact, never a post-hoc rescue. **Why:** keep-recent might win the easy task while throwing away the exact strings coding agents live or die on. If that is how it won, the easy task simply can't see the value of memory — so a loss there isn't evidence the idea is bad, and we press on to the test that can see it. But only a *measured* exact-token failure earns that pass, or the kill rule becomes unfalsifiable.

**R7 — Do not move the main gate off Part 1 onto SWE-bench.** Part 1 stays the cheap, controlled proof and the primary decision gate; R6's advance-to-Part-2 is the single narrow exception (a score-blind null), not a general escape. **Why:** the entire discipline of this project is proving the idea is needed on something cheap and controlled before spending on something expensive and noisy. Moving the real decision onto SWE-bench throws that edge away and rebuilds the expensive thing everyone else does.

**R8 — Be honest about nulls.** Treat "B3 (keep-recent) ties or wins on autoresearch" as a genuinely possible real result, not a bug. Do not tune any policy to beat B3. If the null comes, report it (per §11). **Why:** if simply keeping recent memory is good enough for this task, that is a true finding worth reporting, not a failure to engineer around. Tuning policies until they beat B3 manufactures a win that won't survive contact with the real coding test.
